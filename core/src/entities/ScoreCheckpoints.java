...
public class ScoreCheckpoints {
	...
	private final static int SCORE_DIFF = 10;
	...
	public void init(PreferenceManager prefManager, float screenWidth, float screenHeight, AssetLoader assetLoader) {
		...
		getPosition(Wave.getBallXFromScore(_lastScore, screenWidth), pos);
		_lastScoreX = pos.x;
		_lastScoreY = pos.y;
		getPosition(Wave.getBallXFromScore(_bestScore, screenWidth), pos);
		_bestScoreX = pos.x;
		_bestScoreY = pos.y;
		getPosition(Wave.getBallXFromScore(_bestScore + 1, screenWidth), pos);
		_newBestScoreX = pos.x;
		_newBestScoreY = pos.y;
	}
	public void render(SpriteBatch renderer, float cameraX) {
		if (_lastScore != _bestScore) {
			if (cameraX - _screenWidth * 0.5f < _lastScoreX && cameraX + _screenWidth * 1.5f > _lastScoreX) {
				...
				_assetLoader.ubuntuFont[_assetLoader.SMALL_FONT].setColor(0.0f, 0.0f, 0.0f, 0.8f);
...
//		}
	...
	private void getPosition(float x, Vector2 pos) {
		float y;
		_tmpVector.x = x - _screenWidth / 2.0f;
		_tmpVector.y = _screenHeight * 0.75f - _screenHeight / 2.0f;
		_tmpVector.rotate(Constants.rotation);
		pos.x = _screenWidth / 2.0f + _tmpVector.x;
		pos.y = _screenHeight / 2.0f + _tmpVector.y;
	}
}
